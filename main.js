...
function threadless_download(event, url, path, filename) {
    return new Promise((resolve, reject) => {
        ...
        req.on("data", function (chunk) {
            ...
            if (event != null) event.reply("download-progress", progress);
        });
    ...
    if (total_bytes > 16) {
        ...
        threaded_download(event, url, path, filename, threads, total_bytes)
            ...
            .then(async (res) => {
                ...
                event.reply("download-completed", url);
            })
            .catch((err) => log.info(err));
    }
...
//#region //. User info
ipcMain.on("login", async (event, { login = "", password = "" }) => {
    ...
    if (login == "") {
        event.reply("login-fail", { error: "empty login" });
        return;
    }

    if (password == "") {
        event.reply("login-fail", { error: "empty password" });
        return;
    }

    request(
        {
            uri: "https://deltaminecraft.000webhostapp.com/includes/launcher.login.inc.php",
            method: "POST",
            formData: {
                mailuid: login,
                pwd: password,
        ...
        (error, response) => {
            if (error) {
                ...
                event.reply("login-fail", { error: JSON.stringify(error) });
                return;
            }

            let jsoned = JSON.parse(response.body);
            if (jsoned["error"] != undefined) {
                log.info(`[LOGIN] Error: ${response.body}`);
                event.reply("login-fail", jsoned);
                return;
            } else {
                log.info(`[LOGIN] Success: ${response.body}`);
                sharedObject.userInfo = jsoned;

                keytar.setPassword("Delta", login, password).then((res) => {
                    event.reply("login-success", jsoned);
                    return;
                });
            }
        }
...
ipcMain.on("logout", async (event, { login }) => {
    let res = await keytar.deletePassword("Delta", login).catch((err) => {
        log.error(err);
    });
    sharedObject.userInfo = {};
    event.reply("successful-logout", { res });
});

ipcMain.on("freeze-user-info", async (event) => {
    Object.freeze(sharedObject.userInfo);
});
...
let client = require("discord-rich-presence")("802211656406401074");
...
client.updatePresence(global.sharedObject.rpc);
...
ipcMain.on("download-update", (event, src) => {
    if (!downloading_update) autoUpdater.downloadUpdate();
});
...
ipcMain.on("modpack-launch", async (event, { settings, _modpack_name, min_mem, max_mem, game_dir, username, uuid }) => {
    ...
    win.webContents.send("modpack-log", {
        modpack_name,
        message: `[LAUNCH] <${modpack_name.toUpperCase()}> Final starting command: ${final_command}`,
    });

    sharedObject.launchedModpacks[modpack_name]["process"] = spawn(
        final_command,
        [],
        {
            windowsHide: true,
        ...
        (err) => {
            ...
            win.webContents.send("modpack-update", modpack_name);
        }
    );

    sharedObject.launchedModpacks[modpack_name]["pid"] = -1;
    sharedObject.launchedModpacks[modpack_name]["launched"] = true;

    sharedObject.launchedModpacks[modpack_name]["process"].stdout.on("data", (data) => {
        if (settings["link_consoles"]) {
            win.webContents.send("modpack-log", {
                modpack_name,
                message: `[LAUNCH] <${modpack_name.toUpperCase()}> ${data.toString()}`,
            });
        }

        if (!sharedObject.launchedModpacks[modpack_name]["visible"] && data.toString().split("Starts to replace vanilla recipe ingredients with ore ingredients.").length > 1) {
            sharedObject.launchedModpacks[modpack_name]["visible"] = true;
            win.webContents.send("modpack-log", {
                modpack_name,
                message: `[LAUNCH] Game window opened`,
            });
        ...
        } else if (!sharedObject.launchedModpacks[modpack_name]["loaded"] && data.toString().split("The game loaded in approximately").length > 1) {
            sharedObject.launchedModpacks[modpack_name]["loaded"] = true;
            win.webContents.send("modpack-log", {
                modpack_name,
                message: `[LAUNCH] Game Loaded`,
            });
        ...
        } else if (sharedObject.launchedModpacks[modpack_name]["pid"] == -1 && data.toString().split("Minecraft PID:").length > 1) {
            log.info(data.toString().split("Minecraft PID:")[1]);
            sharedObject.launchedModpacks[modpack_name]["pid"] = data.toString().split("Minecraft PID:")[1];
            win.webContents.send("modpack-log", {
                modpack_name,
                message: `[LAUNCH] PID FOUND`,
            });
    ...
    sharedObject.launchedModpacks[modpack_name]["process"].stderr.setEncoding("utf8");
    sharedObject.launchedModpacks[modpack_name]["process"].stderr.on("data", (data) => {
        win.webContents.send("modpack-log", data.toString());
    });

    sharedObject.launchedModpacks[modpack_name]["process"].on("exit", (error) => {
        if (error)
            win.webContents.send("modpack-log", {
                modpack_name,
        ...
        if (error) {
            win.webContents.send("modpack-exit", {
        ...
        sharedObject.launchedModpacks[modpack_name]["process"] = undefined;
        sharedObject.launchedModpacks[modpack_name]["pid"] = -1;
        sharedObject.launchedModpacks[modpack_name]["launched"] = false;
        sharedObject.launchedModpacks[modpack_name]["visible"] = false;
    });
});
